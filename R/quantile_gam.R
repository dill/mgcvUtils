...
#'
#' @description This method can be used to get the estimated conditional
#' quantiles of the response variable.
#'
#' @param x the output of a `gam` or `bam` call
#' @param probs numeric vector of probabilities with values in (0,1)
#' @param newdata Optional new data frame or list to be passed to `predict.gam`
#' @param w weights
#' @param offset offset
#' @param ... funther arguments to be passed to `predict`
#' @return A matrix where each column is a vector of estimated quantiles
#' responses. The number of rows is equal to the number of responses in the
#' fitted object or in `newdata`.
#' @examples
#' library(mgcvUtils)
#' library(MASS)
